...
pub async fn ai_review(
    ...
    if let Err(e) = db.insert_review(&result, &path) {
        log::warn!("Failed to save review to DB: {}", e);
    }
...
#[tauri::command]
pub fn list_reviews(path: String, db: State<'_, Database>) -> Result<Vec<ReviewResult>, AppError> {
    db.get_reviews_for_project(&path)
}

#[tauri::command]
pub fn delete_review(id: String, db: State<'_, Database>) -> Result<(), AppError> {
    db.delete_review(&id)
}
...
pub async fn generate_commit_msg(
    ...
    if !config.enabled {
        return Err(AppError::Llm(
            "AI is not enabled. Go to Settings to configure LLM.".to_string(),
        ));
    }
